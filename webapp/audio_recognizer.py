...
scaler = load('webapp/models/std_scaler.bin')  # load pretrained SciKit StandardScaler
model_audio = keras.models.load_model('webapp/models/SER_model_without_CREMA_ES.h5')

# model_audio=keras.models.load_model('serrr\SER_model_.h5')
...
def extract_audio_features(data, sample_rate):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig("webapp\diagrams\MelSpec.png")
        ...
        plt.title('Waveplot')
        plt.savefig('webapp\\diagrams\\Waveplot.png')
        plt.close('all')
    except ValueError:
        pass
    return result
